...
export default class ProductDetails extends ProductDetailsBase {
    ...
    setProductVariant() {
        ...
        if (productVariant) {
            ...
            if (view.attr("data-event-type")) {
                ...
                const productName = view.find(".product-view__title__header .heading")[0].innerText.replace(/"/g, "\\$&"); // Updated to reflect new HTML structure / classname
    ...
    productOptionsChanged(event) {
        ...
        utils.api.productAttributes.optionChange(productId, $form.serialize(), "products/bulk-discount-rates", (err, response) => {
            ...
            if (!this.checkIsQuickViewChild($form)) {
                const $context = $form.parents(".product-view").find(".productView-info");
    ...
    addProductToCart(event, form) {
        ...
        utils.api.cart.itemAdd(normalizeFormData(new FormData(form)), (err, response) => {
            ...
            event.initEvent("openMc&CrossSellDrawer", true, true);

            // Listen for the event.
            addToCartButton.addEventListener("openMc&CrossSellDrawer", (e) => {}, false);
    ...
    updateProductDetailsData() {
        ...
        for (const formDataItem of formDataItems) {
            ...
            if (name.match(/attribute/)) {
                ...
                productDetails.optionSelections = productDetails?.optionSelections ? [...productDetails.optionSelections, productOption] : [productOption];
            }
        }
